import Link from 'next/link'

export function Footer() {
  return (
    <footer className="border-t border-gray-800 bg-black mt-auto">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          {/* Brand Column */}
          <div className="md:col-span-2">
            <h3 className="text-2xl font-bold text-primary-red mb-3">1908 Shop</h3>
            <p className="text-gray-400 mb-4 max-w-md">
              Maglie da calcio personalizzate per veri appassionati. Ordina direttamente su
              Instagram con la massima semplicità.
            </p>
            <div className="flex gap-4">
              <a
                href="https://instagram.com/1908shop_"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-gray-900 hover:bg-primary-red rounded-full flex items-center justify-center transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://facebook.com"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-gray-900 hover:bg-primary-red rounded-full flex items-center justify-center transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-semibold mb-4 text-white">Link Rapidi</h4>
            <ul className="space-y-2 text-sm">
              <li>
                <Link
                  href="/products"
                  className="text-gray-400 hover:text-primary-red transition-colors"
                >
                  Tutti i Prodotti
                </Link>
              </li>
              <li>
                <Link
                  href="/mystery-box"
                  className="text-gray-400 hover:text-primary-red transition-colors"
                >
                  Mystery Box
                </Link>
              </li>
              <li>
                <Link
                  href="/products?competition=Serie+A"
                  className="text-gray-400 hover:text-primary-red transition-colors"
                >
                  Serie A
                </Link>
              </li>
              <li>
                <Link
                  href="/products?category=retro"
                  className="text-gray-400 hover:text-primary-red transition-colors"
                >
                  Maglie Vintage
                </Link>
              </li>
            </ul>
          </div>

          {/* Info */}
          <div>
            <h4 className="font-semibold mb-4 text-white">Informazioni</h4>
            <ul className="space-y-2 text-sm">
              <li>
                <a href="#" className="text-gray-400 hover:text-primary-red transition-colors">
                  Come Ordinare
                </a>
              </li>
              <li>
                <a href="#" className="text-gray-400 hover:text-primary-red transition-colors">
                  Spedizioni
                </a>
              </li>
              <li>
                <a href="#" className="text-gray-400 hover:text-primary-red transition-colors">
                  Personalizzazione
                </a>
              </li>
              <li>
                <a href="#" className="text-gray-400 hover:text-primary-red transition-colors">
                  FAQ
                </a>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-gray-800 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm">
            <p className="text-gray-500">
              © {new Date().getFullYear()} 1908 Shop. Tutti i diritti riservati.
            </p>
            <p className="text-xs text-gray-600 text-center md:text-right max-w-md">
              <strong className="text-gray-500">Disclaimer:</strong> Questo è un sito vetrina. Gli
              ordini vengono completati tramite Instagram DM. Attività non ufficiale e non affiliata
              con club sportivi.
            </p>
          </div>
        </div>
      </div>
    </footer>
  )
}
